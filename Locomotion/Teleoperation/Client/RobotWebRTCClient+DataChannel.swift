...
extension RobotWebRTCClient: LKRTCDataChannelDelegate {
    ...
    func sendVelocity(velocityX: Double, velocityY: Double, angularVelocity: Double) {
        let robotVelocityX = velocityY
        let robotVelocityY = -velocityX
        let robotAngularVelocity = -angularVelocity
        let json = String(format: "{\"vx\": %.2f, \"vy\": %.2f, \"omega\": %.2f}",
                          robotVelocityX, robotVelocityY, robotAngularVelocity)
        sendCommand(json)
    }
}
